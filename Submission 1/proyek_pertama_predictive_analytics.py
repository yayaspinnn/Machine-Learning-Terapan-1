# -*- coding: utf-8 -*-
"""3 - Proyek Pertama : Predictive Analytics.ipynb
...
report_ExtraTreesClassifier = classification_report(y_true=y_test, y_pred=ExtraTreesClassifier.predict(X_test))
print(report_ExtraTreesClassifier)

confusion_ExtraTreesClassifier = confusion_matrix(y_true=y_test, y_pred=ExtraTreesClassifier.predict(X_test))
plt.figure(figsize=(8, 8))
sns.heatmap(confusion_ExtraTreesClassifier,
            xticklabels=["Positive", "Negative"],
            yticklabels=["Positive", "Negative"],
            annot=True, 
            annot_kws={"fontsize": "xx-large"},
...
ExtraTreesClassifier.fit(X_train, y_train)
fig = go.Figure(go.Bar(
            x=ExtraTreesClassifier.feature_importances_,
            y=X_train.columns,
            orientation='h', marker_color='steelblue'))
fig.update_layout(title='<b>Estimating feature importance through the GradientBoostingClassifier model', title_x=0.5, 
                 xaxis_title="Feature importance", yaxis_title='Feature', barmode='stack', yaxis={'categoryorder':'total ascending'})
...
# plt.figure()
# sns.boxplot(data=dataset, palette='rocket')
# plt.xticks(rotation=90)

# #correlation matrix
# corrmat = dataset.corr()
# plt.figure(figsize=(10,10))  
# sns.heatmap(corrmat,annot=True, linewidth=0.5, cmap='rocket')
